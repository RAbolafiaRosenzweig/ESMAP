...
for($year=$startyear; $year<=$endyear; $year++) {
  ...
  for ($month=1; $month<=12; $month++){
    $daysinmonth=$days[$month];
	    ...
	    $outdir=sprintf("/Volumes/REESEN/SMAP/Validation_Data/NLDAS2/Mosaic/%04d/%02d/",$year,$month);
	 unless(-e $outdir) {
            $cmd="mkdir -p $outdir";
            print "$cmd\n";
            system($cmd);
  ...
  $IN_FILE=sprintf("/Volumes/LabShare/NLDAS2/MOSAIC/%04d/%02d/NLDAS_MOSAIC0125_H.A%04d%02d%02d.%02d00.nc",$year,$month,$year,$month,$day,$hour);



  #copy over file to new directory and remove un-needed variables          
  $Out_File=sprintf("/NLDAS_MOSAIC0125_H..A%04d%02d%02d.%02d00.nc",$year,$month,$day,$hour);
